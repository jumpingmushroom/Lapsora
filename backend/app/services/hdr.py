...
def _fuse_hdr_frames(frame_paths: list[str], output_path: str, quality: int) -> dict:
    """Read the bracketed frames, merge via Mertens fusion, white-balance and
    save. CPU-bound (OpenCV + numpy) — call via ``asyncio.to_thread`` so it
    doesn't block the event loop. Returns dict with width, height, file_size.
    """
